 ...
 * FAQ content.
 *
 * Every `confirmed` entry below is the client's own answer (see /faqs.txt at
 * the repo root), edited for prose but never for substance. Nothing here is
 * inferred. Where an answer draws on a fact established in a *different*
 * answer — that the deposit is spent on materials immediately, that delivery
 * and teardown are in house, that setup time is the real constraint on scale —
 * it is doing so on purpose, so the set reads as one voice rather than twenty
 * disconnected replies.
 *
 * REVISED 2026-08-28 against a second round of client answers. Five entries
 * did not just gain detail, they reversed. Anything cached, printed, or quoted
 * from the earlier set is now wrong:
 *
 *   1. Deposit — was "half the quote up front, balance due on completion".
 *      Now 25% minimum up front, with the balance due two weeks before the
 *      event rather than after it.
 *   2. Guest capacity — was a hard ceiling of 150. There is no ceiling; the
 *      client has run 200+ and the real constraint is setup time, not size.
 *      The old answer was turning away work they actually want.
 *   3. POs and Net-30 — was "yes, if scheduled well in advance". Now no,
 *      because the deposit is spent on supplies immediately and neither
 *      instrument settles early enough to buy them.
 *   4. Concession staffing — was "staffing is included in the rental price".
 *      It is not. Machines go out self-serve; an attendant is $10/hour on top
 *      of the $125/day. This one was quoting a price for something the client
 *      does not give away, so it mattered most.
 *   5. Extra machine supplies — was $60 per 40 people. Now $5 per additional
 *      guest.
 *
 * Every question that carried a `todo` placeholder now has a real answer, so
 * FAQ_OPEN_COUNT is 0. `todo` entries render with a badge in development and
 * are filtered out of production builds entirely — a question with no answer
 * is worse than no question at all. The mechanism stays in place for the next
 * question a visitor asks that the client hasn't answered yet.
 *
 * TODO(client): one wording check on "Day-Of Coordination vs Full-Service
 * Planning", carried over unresolved. As written, Day-Of is the richer tier
 * (on-site coordinator, vendor point of contact) and Full-Service is setup
 * only. That inverts what those two terms mean almost everywhere else in the
 * industry, so couples self-selecting from this answer may pick the wrong one.
 * The answer states exactly what you wrote; consider renaming the tiers.
 */

export interface FaqItem {
  question: string;
  answer: string;
  /** Rendered as a list under `answer`. Used where a tier has a contents list. */
  bullets?: string[];
  /** Text placed after the bullet list. */
  outro?: string;
  /** Heading this item files under. Items are grouped in source order. */
  group: string;
